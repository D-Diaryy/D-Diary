...
class EditPhotoPatient extends StatefulWidget {
  final doc;
  const EditPhotoPatient({this.doc});

  @override
  State<EditPhotoPatient> createState() => _EditPhotoPatientState();
}

class _EditPhotoPatientState extends State<EditPhotoPatient> {
  ...
  Future updatePhoto() async{
    ...
    try {
      await FirebaseFirestore.instance.collection('users').doc(currentUser!.userID).collection('gallery').doc(widget.doc.id).update({
      ...
      String title = "Patient ${currentUser!.userName} Edit picture info";
      String body = "Description : ${descriptionController.text.trim()} and Title : ${titleController.text.trim()}";
