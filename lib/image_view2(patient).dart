...
class ImageViewPatient2 extends StatefulWidget {
  String? category;
  var doc;
  ImageViewPatient2({this.category,this.doc});

  @override
  State<ImageViewPatient2> createState() => _ImageViewPatient2State();
}

class _ImageViewPatient2State extends State<ImageViewPatient2> {
                    ...
                    Container(
                      ...
                      child: Center(
                        child: ElevatedButton(
                          onPressed: ()async{
                            final imgRef = await FirebaseFirestore.instance.collection('users').doc(currentUser!.userID).collection('trashBin').doc(widget.doc.id);
                            final getPic = await imgRef.get().then((value) => value.data());
                            final curPic = await Picture.fromMap(getPic);
                            await FirebaseFirestore.instance.collection('users').doc(currentUser!.userID).collection('gallery').add(curPic.toMap());
                            await imgRef.delete();
                            String title = "Patient ${currentUser!.userName} Restore a picture";
                            String body = "${widget.doc['Title']} has been restored.";
                            await Notify(title: title, body: body).Carers(null);
                            Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=>RecentlyDeletedPatient()));
                          },
                          child: Text(
                            'restore',
                            textAlign: TextAlign.center,
                                ...
                                return AlertDialog(
                                  ...
                                  actions: <Widget>[
                                    ...
                                    TextButton(
                                      ...
                                      onPressed: () async{
                                        String title = "Patient ${currentUser!.userName} Permanently delete a picture";
                                        String body = "${widget.doc['Title']} has been Permanently deleted.";
                                        Notify(title: title, body: body).Carers(null);
                                        await FirebaseFirestore.instance.collection('users').doc(currentUser!.userID).collection('trashBin').doc(widget.doc.id).delete();
                                        await FirebaseStorage.instance.refFromURL(widget.doc["Image Link"]).getMetadata().then((value) {
                                          var sizeInBytes= value.size;
                                          DatabaseService.updateStorageUsageDec(sizeInBytes, currentUser);
                                        });
